...
const ExpensesDateFilter = ({ dateFilter, setDateFilter, showDateFilter, setShowDateFilter }) => {
  const handleDateFilterChange = (type) => {
    ...
    switch (type) {
      case 'today':
        startDate = startOfDay(today).toISOString().split("T")[0];
        endDate = endOfDay(today).toISOString().split("T")[0];
        break;
      case 'week':
      ...
      case 'all':
        ...
        break;
      case 'custom':
        startDate = dateFilter.startDate || today.toISOString().split("T")[0];
        endDate = dateFilter.endDate || today.toISOString().split("T")[0];
        break;
      default:
        startDate = '';
        endDate = '';
