...
const DebtsSummary = ({ doc, data, clients, products, dateFilter, addTable, yPosition }) => {
  ...
  // Separate debts by product type (Straws and Toilet Paper)
  const strawsDebts = data.debts.filter(debt => {
    const product = products.find(p => p.id === debt.productId || (debt.saleId && p.id === data.sales.find(s => s.id === debt.saleId)?.product?.productId));
    return product && product.name.toLowerCase().includes("straw");
  });

  const toiletPaperDebts = data.debts.filter(debt => {
    const product = products.find(p => p.id === debt.productId || (debt.saleId && p.id === data.sales.find(s => s.id === debt.saleId)?.product?.productId));
    return product && product.name.toLowerCase().includes("toilet paper");
  });

  // Prepare data for Straws debts table
  const strawsDebtsData = sortedData(strawsDebts).map((item) => {
  ...
  // Prepare data for Toilet Paper debts table
  ...
  // Add total for Straws Debts
  const strawsTotal = strawsDebts.reduce((sum, debt) => sum + (parseFloat(debt.amount) || 0), 0);
  doc.setFontSize(12);
  doc.setFont("times", "bold");
  doc.setTextColor(15, 23, 42);
  doc.text(`Total Straws Debts: ${strawsTotal.toLocaleString()} UGX`, 15, yPosition);
  yPosition += 10;
  ...
  // Add total for Toilet Paper Debts
  const toiletPaperTotal = toiletPaperDebts.reduce((sum, debt) => sum + (parseFloat(debt.amount) || 0), 0);
  doc.setFontSize(12);
  doc.setFont("times", "bold");
  doc.setTextColor(15, 23, 42);
  doc.text(`Total Toilet Paper Debts: ${toiletPaperTotal.toLocaleString()} UGX`, 15, yPosition);
  yPosition += 10;
